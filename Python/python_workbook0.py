...
#!,директива позволяет указать версию Python для исполнения, кодировку, сделать файл sc исполняемым сценарием в lunux/Win7+,должна стоять первой
...
#содержит дескрипторные файлы
    ...
    #запуск комманд оболочки и(в отличие от .system) предоставляет интерфейс(подобный файлам) для чтение/запись в их стандартные потоки данных в отдельном процессе,подключается к потокам вывода программы
...
как сменить кодироку в консоли винды на постоянку?
    .system
    #находится в модуле nt? - да, все верно
    #запуск комманды оболочки в отдельном процессе, ждет завершения процесса прежде чем возобновить поток выполнения
    #возвращает 0 в случае успеха, 1 в случае неудачи, например отсутсвие команды;не возвращает вывод и не открывает консольные окна
        ...
        os.system('dir')  >> 0
nt
#модуль, использующийся os на win
    .system()
    #Это os.system()
способы отладки
